...
const buildHistory = (entries: Array<{ status: string; changedByClerkUserId?: string; note?: string; changedAt: string }>) =>
  entries.map((entry) => ({
    ...entry,
...
const seedDatabase = async () => {
  try {
    ...
    const referralByPatientAndCreatedAt = createdReferrals.reduce<Record<string, (typeof createdReferrals)[number]>>((accumulator, referral) => {
